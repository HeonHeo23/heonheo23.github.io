...
export default function ResearchDetailPage() {
  ...
  if (!paper) {
    return (
      <main className="page-shell">
        ...
        <Link href="/research" className="link mt-6 inline-block text-primary">Back to research</Link>
      </main>
    );
  }
  ...
  return (
    <main className="page-shell">
      <nav className="breadcrumbs eyebrow mb-10 w-full text-primary" aria-label="Breadcrumb">
        <ul className="flex w-full min-w-0">
          <li className="shrink-0"><Link href="/research">Research</Link></li>
          <li
      ...
      <article className="article-content">
        <div className="mb-10 border-b-2 border-base-content pb-10">
          <div className="flex flex-wrap gap-2">
            <span className="badge badge-primary rounded-none font-mono font-bold uppercase tracking-wider">
              {paper.type}
            </span>
            {paper.tags.map((tag) => (
              <span key={tag} className="badge badge-outline rounded-none">{tag}</span>
            ))}
          </div>
        ...
        <dl className="mb-10 grid gap-px border-2 border-base-content bg-base-content sm:grid-cols-2">
          <div className="bg-base-100 p-4">
            <dt className="eyebrow text-primary">Published</dt>
            <dd className="mt-2 font-mono text-sm font-bold">{paper.publishedDate}</dd>
          </div>
          <div className="bg-base-100 p-4">
          ...
          <div className="bg-base-100 p-4">
            ...
            <dd className="mt-2 font-mono text-sm font-bold">Vol. {paper.volume}</dd>
          </div>
          <div className="bg-base-100 p-4">
            <dt className="eyebrow text-primary">Article / paper number</dt>
            <dd className="mt-2 font-mono text-sm font-bold">{paper.identifier}</dd>
          </div>
          <div className="bg-base-100 p-4">
          ...
          <div className="bg-base-100 p-4">
            ...
            <dd className="mt-2 font-mono text-sm font-bold">{paper.publisher}</dd>
          </div>
        ...
        <section aria-labelledby="abstract-heading">
          <p id="abstract-heading" className="eyebrow mb-6 text-primary">Abstract</p>
          <div className="space-y-8">
            {Object.entries(paper.abstractSections).map(([label, text]) => (
              <div key={label}>
                ...
                <p className="mt-2 text-lg leading-relaxed text-base-content/80">{text}</p>
              </div>
            ))}
          </div>
        </section>
